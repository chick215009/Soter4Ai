...
import cn.edu.nju.core.stereotype.analyzer.TypeAnalyzer;
import cn.edu.nju.core.stereotype.information.TypeInfo;
import cn.edu.nju.core.stereotype.rules.TypeStereotypeRules;
...
public class StereotypedType extends TypeStereotypeRules implements StereotypedElement {
    private Set<IVariableBinding> fields;//成员变量
...
//                System.out.println(fragment.resolveBinding().getType().getQualifiedName());
...
//        this.completeReport();
    ...
    private void findSubtypesStereotypes() {
        ...
        for (int length = (types = this.type.getTypes()).length, i = 0; i < length; ++i) {
            ...
            // 注释如下语句，否则执行Main.java时会报空指针错误
            if (subtype.resolveBinding() != null && subtype.resolveBinding().isClass()) {
