...
public class JavaMethodBodyConverter extends ASTVisitor {
        ...
        if(forStatement.initializations != null && forStatement.initializations.length > 0) {
        	for(Statement initStatement : forStatement.initializations) {
        		push(
        			...
        			initStatement.concreteStatement()
        		);
        ...
        if(forStatement.increments != null && forStatement.increments.length > 0) {
        	for(Statement incrementStatement : forStatement.increments) {
        		push(
        			...
        			incrementStatement.concreteStatement()
        		);
    ...
    public boolean visit(IfStatement ifStatement, BlockScope scope) {
        ...
        push(JavaEntityType.IF_STATEMENT, expression, ifStatement.sourceStart(), ifStatement.sourceEnd(), ifStatement);
        if (ifStatement.thenStatement != null) {
            push(
                    JavaEntityType.THEN_STATEMENT,
                    expression,
                    ifStatement.thenStatement.sourceStart(),
                    ifStatement.thenStatement.sourceEnd(), ifStatement.thenStatement);
    ...
    public boolean visit(LocalDeclaration localDeclaration, BlockScope scope) {
        ...
        push(fASTHelper.convertNode(localDeclaration), localDeclaration.toString(), start, end + 1, localDeclaration);
        return false;
    }
    ...
    public boolean visit(TryStatement node, BlockScope scope) {
        preVisit(node);
        pushEmptyNode(node);
        push(JavaEntityType.BODY, "", node.tryBlock.sourceStart(), node.tryBlock.sourceEnd(), node.tryBlock);
    ...
    private void visitFinally(TryStatement node, BlockScope scope) {
        if (node.finallyBlock != null) {
            push(JavaEntityType.FINALLY, "", node.finallyBlock.sourceStart(), node.finallyBlock.sourceEnd(), node.finallyBlock);
    ...
    private void pushValuedNode(ASTNode node, String value) {
        push(fASTHelper.convertNode(node), value, node.sourceStart(), node.sourceEnd(), node);
    }

    private void pushEmptyNode(ASTNode node) {
        push(fASTHelper.convertNode(node), "", node.sourceStart(), node.sourceEnd(), node);
    }

    private void push(EntityType label, String value, int start, int end, ASTNode node) {
        Node n = new Node(label, value.trim());
        n.setEntity(new SourceCodeEntity(value.trim(), label, new SourceRange(start, end), node, "", null));
